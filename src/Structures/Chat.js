...
class Chat extends Base {
     ...
     * @returns {Promise<Boolean>} result
     */
    async clearMessages(keepStarred) {
        return this.client.playPage.evaluate(({ chatId, keepStarred }) => {
            return window.WPP.chat.clear(chatId, keepStarred);
        }, { chatId: this.id._serialized, keepStarred });
    }
     ...
     */
    async delete() {
        return this.client.playPage.evaluate(chatId => {
            return window.WPP.chat.delete(chatId);
        }, this.id._serialized);
    }
    ...
    async fetchMessages(searchOptions) {
        let messages = await this.client.playPage.evaluate(async ({ chatId, searchOptions }) => {
            const msgFilter = (m) => {
                if (m.isNotification) {
            ...
            if (searchOptions && searchOptions.limit > 0) {
                while (msgs.length < searchOptions.limit) {
                    const loadedMessages = await window.WPP.whatsapp.ChatStore.loadEarlierMsgs(chat);
            ...
            return msgs.map(m => window.WAJS.getMessageModel(m));
    ...
    async sendStateTyping() {
        return this.client.playPage.evaluate(chatId => {
            window.WAJS.sendChatstate('typing', chatId);
            return true;
        }, this.id._serialized);
    }
    ...
    async sendStateRecording() {
        return this.client.playPage.evaluate(chatId => {
            window.WAJS.sendChatstate('recording', chatId);
            return true;
        }, this.id._serialized);
    }
    ...
    async clearState() {
        return this.client.playPage.evaluate(chatId => {
            window.WAJS.sendChatstate('stop', chatId);
            return true;
        }, this.id._serialized);
    }
