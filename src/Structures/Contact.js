...
class Contact extends Base {
    ...
    async block() {
        ...
        await this.client.playPage.evaluate(async (contactId) => {
            const contact = window.WPP.whatsapp.ContactStore.get(contactId);
            await window.WPP.whatsapp.functions.blockContact(contact);
        }, this.id._serialized);
    ...
    async unblock() {
        ...
        await this.client.playPage.evaluate(async (contactId) => {
            const contact = window.WPP.whatsapp.ContactStore.get(contactId);
            await window.WPP.whatsapp.functions.unblockContact(contact);
        }, this.id._serialized);
    ...
    async getAbout() {
        const about = await this.client.playPage.evaluate(async (contactId) => {
            const wid = window.WPP.whatsapp.WidFactory.createWid(contactId);
            return window.WPP.whatsapp.StatusStore.find(wid);
        }, this.id._serialized);
